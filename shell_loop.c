 ...
 */
int hsh(info_t *info_struct, char **argument_vector)
{
    ssize_t r = 0;
    int builtin_ret = 0;
    ...
    while (r != -1 && builtin_ret != -2)
    {
        ...
        if (interactive(info_struct))
            _puts("$ ");
        _eputchar(BUF_FLUSH);
        r = get_input(info_struct);
        if (interactive(info_struct))
            _putchar('\n');
        else if (r != -1)
        {
            set_info(info_struct, argument_vector);
            builtin_ret = find_builtin(info_struct);
            if (builtin_ret == -1)
...
void find_cmd(info_t *info_struct)
{
    ...
    for (int i = 0, k = 0; info_struct->arg[i]; i++)
